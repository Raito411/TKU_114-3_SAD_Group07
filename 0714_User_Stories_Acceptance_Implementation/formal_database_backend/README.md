...
| `notifications` | 每位使用者信箱中的通知，賣方核准或更新面交資訊後會寫入買方信箱 |

## 啟動

```bash
cd /Users/artis/Documents/GitHub/TKU_114-3_SAD_Group07/0714_User_Stories_Acceptance_Implementation/formal_database_backend
python3 server.py
```
...
| `POST` | `/api/purchase-requests` | 登入學生送出購買需求 |
| `GET` | `/api/purchase-requests` | 查詢自己送出與收到的購買需求，賣方可看到要求人姓名與發送要求時間 |
...
此版本已具備正式資料庫、自建註冊/登入頁、登入 session、學生刊登書籍、學生送出購買需求、賣方核准需求、賣方標記交易失敗、使用者信任度、信任度 0 停權與每位使用者自己的信箱。系統不串接學校正式單一登入、正式學校帳號或真實身分驗證。
