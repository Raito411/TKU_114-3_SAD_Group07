...
function getVisibleRequestText(book) {
  ...
  if (isOwnListing(book)) return (book.purchaseRequests || []).length ? `${(book.purchaseRequests || []).length} 筆要求` : "尚未提出";
  return "尚未提出";
}
...
function renderPurchaseRequestsMarkup(book) {
  const requests = book.purchaseRequests || [];
    ...
    .map((book) => `
      <article class="transaction-card">
        <div>
          ...
          <p>購買需求：${(book.purchaseRequests || []).length} 筆</p>
