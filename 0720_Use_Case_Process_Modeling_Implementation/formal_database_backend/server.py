...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            elif parsed.path.startswith("/api/purchase-requests/") and parsed.path.endswith("/cancel"):
                request_id = int(parsed.path.split("/")[3])
                self.cancel_purchase_request(request_id)
        ...
        with connect_db() as connection:
            ...
            connection.execute(
                ...
                (
                    ...
                    cursor.lastrowid,
                    "收到新的購買需求",
                    f"{buyer['name']} 已對「{book['title']}」送出購買需求。",
                ),
            )
    ...
    def list_purchase_requests(self):
        with connect_db() as connection:
            ...
            as_seller = connection.execute(
                ...
                WHERE b.seller_id = ? AND pr.status <> '已取消'
                ORDER BY pr.created_at DESC
                """,
                (user["id"],),
            ).fetchall()
    ...
    def cancel_purchase_request(self, request_id):
        with connect_db() as connection:
            buyer = self._auth_user(connection)
            if not buyer:
                self._send_json(401, {"error": "請先登入"})
                return
            ...
            request = connection.execute(
                """
                SELECT pr.*, b.status AS book_status
                FROM purchase_requests pr
                JOIN books b ON b.id = pr.book_id
                WHERE pr.id = ?
                """,
            ...
            if request["buyer_id"] != buyer["id"]:
                self._send_json(403, {"error": "只有提出需求的買方可以取消"})
                return
            if request["status"] != "待賣方回覆" or request["book_status"] != "未交易":
                self._send_json(409, {"error": "只能取消尚未被賣方核准的購買需求"})
                return
        ...
        self._send_json(200, {"message": "購買需求已取消"})
